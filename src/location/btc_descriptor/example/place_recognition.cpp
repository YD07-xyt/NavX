...
int main(int argc, char **argv) {
  ...
  node->declare_parameter<double>("cloud_overlap_thr", 0.5);
  node->declare_parameter<bool>("read_bin", true);
  node->declare_parameter<std::string>("setting_path", "");
  node->declare_parameter<std::string>("pcds_dir", "");
  node->declare_parameter<std::string>("pose_file", "");
  ...
  rclcpp::WallRate loop_rate(10000);
  rclcpp::WallRate slow_rate(1000);
  ...
  RCLCPP_INFO(node->get_logger(), "%s", print_msg.c_str());
    ...
    for (size_t submap_id = 0; submap_id < pose_list.size(); ++submap_id) {
      ...
      if (read_bin) {
        ...
        if (reader.read(pcd_file, *cloud) == -1) {
          RCLCPP_ERROR(node->get_logger(), "Couldn't read file %s",
                       pcd_file.c_str());
          continue;
        }
        auto t_load_end = std::chrono::high_resolution_clock::now();
