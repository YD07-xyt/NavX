...
class IsGameTimeLess : public BT::ConditionNode {
public:
  IsGameTimeLess(const std::string &name, const BT::NodeConfig &config)
  ...
  BT::NodeStatus tick() override {
    ...
    if (!getInput<int>("game_time", game_time_)) {
      spdlog::warn(
          "[IsGameTimeLess] failed read game_time from input port");
      // 尝试直接从黑板读取
      int blackboard_time;
    ...
    if (!getInput<int>("threshold", threshold_)) {
      spdlog::warn("IsGameTimeLess: 读取 threshold 失败");
      return BT::NodeStatus::FAILURE;
    }

    if (game_time_ < threshold_) { 
      spdlog::debug("[IsGameTimeLess] 时间<阈值");
