...
#include "src/serial/packet_typedef.h"
#include "src/serial/serial.h"
#include "src/tools/plotter.hpp"
namespace io {
class SerialNode {
public:
  SerialNode(const std::string &serial_name, int &baud_rate, int &max_try,
    ...
    this->is_open_serial = this->serial_driver->open_socket(
        socket_receive_name_, socket_send_name_);
    // this->is_open_serial =
...
private:
  ...
  void cmd_callback(geometry_msgs::msg::Twist::SharedPtr cmd_data) {
    ...
    //             send_cmd.crc16);
    serial_driver->send_socket(send_cmd);
    plotter_debug_cmd(now,send_cmd.v_x,send_cmd.v_y,send_cmd.w_z);
  };
