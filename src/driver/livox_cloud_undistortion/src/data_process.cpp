...
void ImuProcess::IntegrateGyr(const std::vector<sensor_msgs::msg::Imu::SharedPtr> &v_imu) 
{
  /// Reset gyr integrator
  gyr_int_.Reset(last_lidar_->header.stamp.sec, last_imu_);
  /// And then integrate all the imu measurements
  for (const auto &imu : v_imu) {
    gyr_int_.Integrate(imu);
  }
  spdlog::info("integrate rotation angle [x, y, z]: [%.2f, %.2f, %.2f]",
...
{
  if(meas.imu.empty()){
    spdlog::warn("meas.imu.empty()");
  }

    if(meas.lidar==nullptr){
    spdlog::warn("meas.lidar= nullptr");
  }
  spdlog::debug("Process lidar at time: {:4f}, {} imu msgs from {:4f} to {:4f}",
            meas.lidar->header.stamp.sec, meas.imu.size(),
            meas.imu.front()->header.stamp.sec,
            meas.imu.back()->header.stamp.sec);
  ...
  if (b_first_frame_) {
    ...
    spdlog::warn("The very first lidar frame");
  ...
  dt_l_c_ =
      pcl_in_msg->header.stamp.sec - last_lidar_->header.stamp.sec;
  //// Get input pcl
...

  {
    static auto pub_UndistortPcl =
        nh->create_publisher<sensor_msgs::msg::PointCloud2>("/livox_first_point", 100);
  ...
  {
    static auto pub_UndistortPcl =
        nh->create_publisher<sensor_msgs::msg::PointCloud2>("/livox_undistort", 100);
    sensor_msgs::msg::PointCloud2 pcl_out_msg;
  ...
  {
    static auto pub_UndistortPcl =
        nh->create_publisher<sensor_msgs::msg::PointCloud2>("/livox_origin", 100);
    sensor_msgs::msg::PointCloud2 pcl_out_msg;
