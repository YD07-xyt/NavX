...
void GyrInt::Reset(double start_timestamp,
                   const sensor_msgs::msg::Imu::SharedPtr &lastimu) {
...
void GyrInt::Integrate(const sensor_msgs::msg::Imu::SharedPtr &imu) {
  /// Init
  if (v_rot_.empty()) {
    if (!(start_timestamp_ > 0)) {
      spdlog::warn("!(start_timestamp_ > 0)");
    }
    if (last_imu_ == nullptr) {
      spdlog::warn("last_imu_ == nullptr");
    }
    /// Identity rotation
    ...
    double dt1 = start_timestamp_ - last_imu_->header.stamp.sec;
    double dt2 = imu->header.stamp.sec - start_timestamp_;
    if (!(dt1 >= 0 && dt2 >= 0)) {
      spdlog::warn("Unreasonable timestamps: last_imu_sec={}, "
                   "start_timestamp={}, imu_sec={}",
                   last_imu_->header.stamp.sec, start_timestamp_,
    ...
    imu_inter->header.stamp = rclcpp::Time(start_timestamp_, RCL_ROS_TIME);
