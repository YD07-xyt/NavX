...
#include <fstream>
namespace bt {
RmDecision::RmDecision(bt::DecisionConfig decision_config): config_(decision_config) {
    ...
    output_blackboard_ = std::make_shared<Outputblackboard>(tree_blackboard_);
    groot2publisher_ptr_ = std::make_unique<BT::Groot2Publisher>(tree_, groot_port);
};
auto RmDecision::tree_tick(
...
void RmDecision::setting_tree_custom_node() {
    ...
    factory_.registerBuilder<bt::Nav2Pose>("Nav2Pose", [this](const std::string& name, const BT::NodeConfig& config) {
    ...
    factory_.registerNodeType<bt::IsEnemyOutpostDestroyed>("IsEnemyOutpostDestroyed");
