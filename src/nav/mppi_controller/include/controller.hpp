...
public:
    ...
    {
        settings_ = settings;
        base_motion_model_type_ = motion_model_type;
    ...
    void updateStaticObstacles(const std::vector<Point2D> & points, const Pose2D & robot_pose) {
        ...
        updateMotionModel(robot_pose, points);
    }

    /** @return 最近障碍物距离 */
    float getMinObstacleDistance() const { return min_obstacle_distance_; }

    /** @return 当前路径偏离程度 */
    float getCurrentPathDeviation() const { return current_path_deviation_; }
    ...
    bool isOmniModeActive() const { return omni_mode_active_; }
    std::string getCurrentMotionModelType() const { return omni_mode_active_ ? "Omni" : base_motion_model_type_; }
...
private:
    ...
    // 全向/差速模式切换相关成员
    std::string base_motion_model_type_ = "DiffDrive";
    bool omni_mode_active_ = false;
    int omni_switch_counter_ = 0;
    float min_obstacle_distance_ = std::numeric_limits<float>::max();
    float current_path_deviation_ = 0.0f;

    /**
     * @brief 更新运动模型（根据障碍物距离和路径偏离程度自动切换）
     */
    ...
    float computeMinObstacleDistance(const Pose2D& robot_pose, const std::vector<Point2D>& obstacles) const
    {
        if (obstacles.empty()) return std::numeric_limits<float>::max();

        float min_dist = std::numeric_limits<float>::max();
        for (const auto& obs : obstacles) {
            float dist = std::hypot(robot_pose.x - obs.x, robot_pose.y - obs.y);
            if (dist < min_dist) min_dist = dist;
        }
        return min_dist;
    }

    float computePathDeviation(const Pose2D& robot_pose) const
    {
        if (path_.empty()) return 0.0f;

        float min_dist = std::numeric_limits<float>::max();
        for (size_t i = 0; i < path_.size(); ++i) {
            float dist = std::hypot(robot_pose.x - path_.x(i), robot_pose.y - path_.y(i));
            if (dist < min_dist) min_dist = dist;
        }
        return min_dist;
    }
    ...
    void switchToOmniMode()
    {
        ...
        auto new_motion_model = std::make_shared<OmniMotionModel>();
        new_motion_model->setAccelerationConstraints(
        ...
        optimizer_->setMotionModel(new_motion_model);
        motion_model_ = new_motion_model;

        if (auto* critic = critic_manager_->getCritic("ConstraintCritic")) {
            auto* cc = dynamic_cast<ConstraintCritic*>(critic);
            if (cc) {
                cc->setParams(4.0f, settings_.base_constraints.vx_max, settings_.base_constraints.vx_min,
                             settings_.base_constraints.vy_max, settings_.base_constraints.wz_max,
                             0.2f, 1);
            }
        }

        std::cout << "[MPPI] Switched to OMNI mode (obstacle_dist=" << min_obstacle_distance_
                  << ", path_dev=" << current_path_deviation_ << ")" << std::endl;
    }

    void switchToDiffMode()
    {
        if (!omni_mode_active_) return;

        omni_mode_active_ = false;
        omni_switch_counter_ = 0;

        optimizer_->softReset();

        std::shared_ptr<MotionModel> new_motion_model;
        if (base_motion_model_type_ == "DiffDrive") {
            new_motion_model = std::make_shared<DiffDriveMotionModel>();
        } else if (base_motion_model_type_ == "Ackermann") {
            new_motion_model = std::make_shared<AckermannMotionModel>(0.2f);
        } else {
            new_motion_model = std::make_shared<DiffDriveMotionModel>();
        }
        ...
        if (auto* critic = critic_manager_->getCritic("ConstraintCritic")) {
            auto* cc = dynamic_cast<ConstraintCritic*>(critic);
            if (cc) {
                int model_type = (base_motion_model_type_ == "Ackermann") ? 2 : 0;
                cc->setParams(4.0f, settings_.base_constraints.vx_max, settings_.base_constraints.vx_min,
                             settings_.base_constraints.vy_max, settings_.base_constraints.wz_max,
                             0.2f, model_type);
            }
        ...
        std::cout << "[MPPI] Restored DIFF mode (obstacle_dist=" << min_obstacle_distance_
                  << ", path_dev=" << current_path_deviation_ << ")" << std::endl;
    }
};
