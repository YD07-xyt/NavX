...
auto FSM::plan(const Eigen::Vector3d &goal_pose,
  ...
  while (path_state_ != PathState::running && retry_count < max_retries) {
    ...
    TrajOpt::TrajectoryOptimizer optimizer(grid_map, astar_traj.optimized_path,
                                           fsm_config_.params_);
    if (!optimizer.plan()) {
...
auto FSM::checkPointEqual(const Eigen::Vector3d &pos1, const Eigen::Vector3d &pos2,
