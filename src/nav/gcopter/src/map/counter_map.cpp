...
#include <map/3d_occ_map/counter_map.h>

namespace rog_map {

void CounterMap::initCounterMap(
    const utils::Vec3i& half_prob_map_size_i, /* The input is half map size, to ensure the map size is always odds*/
    ...
    logger::CounterMap->warn("inflation_ratio: {}", inflation_ratio);
    ...
    utils::Vec3f half_prob_map_size_d = half_prob_map_size_i.cast<double>() * prob_map_resolution;
    // compute the counter map size in double
    utils::Vec3i half_counter_map_size_i =
        (half_prob_map_size_d / counter_map_resolution).cast<int>() + (inflation_step + 1) * utils::Vec3i::Ones();
    ...
    resetLocalMap();
    logger::CounterMap->info("Init successfully, map size: {}, sub_grid_num: {}, unk_thresh: {}.", map_size, md_.sub_grid_num, md_.unk_thresh);
    printMapInformation();
}

void CounterMap::updateGridCounter(const utils::Vec3f& pos, const GridType& from_type, const GridType& to_type) {
    /*
         ...
         * */
    utils::TimeConsuming update_t("updateGridCounter", false);
    map_empty_ = false;
    utils::Vec3i id_l, id_g;
    ...
    if (md_.occupied_cnt[addr] < 0 || md_.occupied_cnt[addr] > md_.sub_grid_num) {
        logger::CounterMap->warn("From type: {}", GridTypeStr[from_type]);
        logger::CounterMap->warn("To type: {}", GridTypeStr[to_type]);
        logger::CounterMap->warn("Occupied counter: {}", md_.occupied_cnt[addr]);
    ...
    if (md_.unknown_cnt[addr] < 0 || md_.unknown_cnt[addr] > md_.sub_grid_num) {
        logger::CounterMap->warn("From type: {}", GridTypeStr[from_type]);
        logger::CounterMap->warn("To type: {}", GridTypeStr[to_type]);
        logger::CounterMap->warn("Unknown counter: {}", md_.unknown_cnt[addr]);
...
bool CounterMap::isUnknown(const utils::Vec3i& id_g) const {
    return isUnknown(getHashIndexFromGlobalIndex(id_g));
}

bool CounterMap::isOccupied(const utils::Vec3i& id_g) const {
    return isOccupied(getHashIndexFromGlobalIndex(id_g));
}

bool CounterMap::isKnownFree(const utils::Vec3i& id_g) const {
