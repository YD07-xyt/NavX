...
#include "map/3d_occ_map/rog_map.h"
...
void ROGMap::init() {
    ...
    if (cfg_.map_sliding_en) {
        mapSliding(utils::Vec3f(0, 0, 0));
        inf_map_->mapSliding(utils::Vec3f(0, 0, 0));
    }
    else {
    ...
    if (cfg_.load_pcd_en) {
        ...
        if (pcl::io::loadPCDFile(pcd_path, *pcd_map) == -1) {
            logger::ROGMap->warn("Load pcd file at: [{}] failed!", cfg_.pcd_name);
        }
        Pose cur_pose;
        cur_pose.first = utils::Vec3f(0, 0, 0);
        updateOccPointCloud(*pcd_map);
        if(cfg_.esdf_en) {
            esdf_map_->updateESDF3D(robot_state_.p);
        }
        logger::ROGMap->info(" -- [ROGMap]Load pcd file success with {} pts.", pcd_map->size());
        map_empty_ = false;
    }
}

bool ROGMap::findNearestCellThat(const bool & is, const GridType& target_type,
    const utils::Vec3f & start_pos,  utils::Vec3f& nearest_pt, const double & max_dis) const {

     utils::Vec3i start_id;
    posToGlobalIndex(start_pos, start_id);
    nearest_pt.setConstant(NAN);


    for(const auto & nei_id: cfg_.spherical_neighbor) {
        const  utils::Vec3i q_id =start_id + nei_id;
         utils::Vec3f q_pos;
...
bool ROGMap::findNearestInfCellThat(const bool & is, const GridType& target_type,
    ...
    logger::ROGMap->warn("findNearestInfCellThat failed to find all {} neighbors at start_pos: {}, target_type: {}, is: {}", cfg_.spherical_neighbor.size(), start_pos.transpose(), target_type, is);
    return false;
}


bool ROGMap::isLineFree(const utils::Vec3f& start_pt, const utils::Vec3f& end_pt,
                        const bool& use_inf_map, const bool& use_unk_as_occ) const {
    if (start_pt.array().isNaN().any() || end_pt.array().isNaN().any()) {
        logger::ROGMap->warn(" Call isLineFree with NaN in start or end pt, return false.");
        return false;
    }
...
bool ROGMap::isLineFree(const utils::Vec3f& start_pt, const utils::Vec3f& end_pt, const double& max_dis,
                        const utils::vec_Vec3i& neighbor_list) const {
    raycaster::RayCaster raycaster;
    raycaster.setResolution(cfg_.resolution);
    utils::Vec3f ray_pt;
    raycaster.setInput(start_pt, end_pt);
...
bool ROGMap::isLineFree(const utils::Vec3f& start_pt, const utils::Vec3f& end_pt, utils::Vec3f& free_local_goal, const double& max_dis,
                        const utils::vec_Vec3i& neighbor_list) const {
    raycaster::RayCaster raycaster;
    raycaster.setResolution(cfg_.resolution);
    utils::Vec3f ray_pt;
...
void ROGMap::updateMap(const PointCloud& cloud, const Pose& pose) {
    utils::TimeConsuming updateMap_timerconsum("updateMap", true);
    updateMap_timerconsum.start();
    if (cfg_.ros_callback_en) {
        logger::ROGMap->warn("ROS callback is enabled, can not insert map from updateMap API.");
        return;
    }
    ...
    if (cloud.empty()) {
        ...
        if (local_cnt++ > 100) {
            logger::ROGMap->warn("No cloud input, please check the input topic.");
            local_cnt = 0;
        }
        return;
    }
