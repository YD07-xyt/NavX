...
#include <map/3d_occ_map/inf_map.h>

namespace rog_map {
// Public Query Function ========================================================================
bool InfMap::isOccupiedInflate(const utils::Vec3f& pos) const {
...
bool InfMap::isOccupiedInflate(const utils::Vec3i& id_g) const {
...
bool InfMap::isKnownFreeInflate(const utils::Vec3f& pos) const {
...
bool InfMap::isUnknownInflate(const utils::Vec3f& pos) const {
    if (!cfg_.unk_inflation_en) {
...
void InfMap::infMapGlobalIndexToPos(const utils::Vec3i& id_g, utils::Vec3f& pos) const {
    globalIndexToPos(id_g, pos);
}

void InfMap::infMapPosToGlobalIndex(const utils::Vec3f& pos, utils::Vec3i& id) const {
...
InfMap::InfMap(rog_map::Config& cfg) {
    ...
    cfg_ = cfg;
    logger::InfMap->info(
        "Init successfully, map size: {}, sub_grid_num: {}, unk_thresh: {}.",
        sc_.map_vox_num,
        md_.sub_grid_num,
        md_.unk_thresh
    );
...
void InfMap::boxSearch(
    ...
    out_points.clear();
    if (map_empty_) {
        logger::InfMap->warn("Map is empty, cannot perform box search.");
        return;
    }
    ...
    if (gt == utils::UNKNOWN) {
        if (!cfg_.unk_inflation_en) {
            out_points.clear();
            logger::InfMap->warn("Unknown inflation is not enabled, cannot perform box search.");
            return;
        }
...
void InfMap::updateInflation(const utils::Vec3i& id_g, const bool is_hit) {
    utils::TimeConsuming tc("updateInflation", false);
    for (const auto& nei: cfg_.inf_spherical_neighbor) {
        const utils::Vec3i& id_shift = id_g + nei;
...
void InfMap::updateUnkInflation(const utils::Vec3i& id_g, const bool is_add) {
    utils::TimeConsuming tc("updateInflation", false);
    if (!cfg_.unk_inflation_en) {
        logger::InfMap->warn("Cannot updateUnkInflation of InfMap when unk_inflation_en is false.");
        return;
    }
    ...
    for (const auto& nei: cfg_.unk_inf_spherical_neighbor) {
        ...
        if (imd_.unk_inflate_cnt[addr] < 0 || imd_.unk_inflate_cnt[addr] > imd_.unk_neighbor_num) {
            logger::InfMap->warn("unk_inflate_cnt: {}", imd_.unk_inflate_cnt[addr]);
            logger::InfMap->warn("unk_neighbor_num: {}", imd_.unk_neighbor_num);
            throw std::runtime_error(" -- [IM] Negative occupancy counter, which should not happened.!");
        }
#endif
    }
...
GridType InfMap::getGridType(const utils::Vec3i& id_g) const {
    ...
    if (imd_.occ_inflate_cnt[addr] > 0) {
        ...
        return utils::KNOWN_FREE;
    }
}

GridType InfMap::getGridType(const utils::Vec3f& pos) const {
