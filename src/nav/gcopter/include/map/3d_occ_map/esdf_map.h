...
namespace rog_map {
    ...
    public:
        ...
        void initESDFMap(
                ...
                const utils::Vec3f &local_update_box,
                const bool &map_sliding_en,
                const double &sliding_thresh,
                const utils::Vec3f &fix_map_origin,
                const double &unk_thresh);

        void getUpdatedBbox(utils::Vec3f & box_min,utils::Vec3f & box_max)const;

        void resetLocalMap() override;

        double getDistance(const utils::Vec3f &pos) const;

        double getDistance(const utils::Vec3i &id_g) const;

        void updateESDF3D(const utils::Vec3f &cur_odom);
        ...
        void getPositiveESDFPointCloud(const utils::Vec3f &box_min_d, const utils::Vec3f &box_max_d,
                                     const double &visualize_z, pcl::PointCloud<pcl::PointXYZI> & pcl_pc);

        void getNegativeESDFPointCloud(const utils::Vec3f &box_min_d, const utils::Vec3f &box_max_d,
                                     const double &visualize_z, pcl::PointCloud<pcl::PointXYZI> & pcl_pc);
    ...
    private:
        ...
        utils::Vec3i half_local_update_box_i_;
        utils::Vec3i update_local_map_min_i_, update_local_map_max_i_;

        std::mutex update_esdf_mtx;


        void triggerJumpingEdge(const utils::Vec3i &id_g, const rog_map::GridType &from_type,
        ...
        void getSurroundPts(const utils::Vec3f& pos, utils::Vec3f pts[2][2][2], utils::Vec3f & diff);
