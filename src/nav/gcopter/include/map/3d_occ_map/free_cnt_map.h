...
namespace rog_map {
    ...
    public:
        ...
        FreeCntMap(const utils::Vec3i &half_map_size_i,
                   const double &resolution,
                   const bool &sliding_en,
                   const double &sliding_thresh,
                   const utils::Vec3f &fix_map_origin) : SlidingMap(half_map_size_i,
                                                         resolution,
                                                         sliding_en,
                                                         sliding_thresh,
                                                         fix_map_origin) {
        ...
        int getFreeCnt(const utils::Vec3f &pos) {
            return neighbor_free_cnt[getHashIndexFromPos(pos)];
        }

        int getFreeCnt(const utils::Vec3i &id_g) {
            return neighbor_free_cnt[getHashIndexFromGlobalIndex(id_g)];
        }

        void updateFrontierCounter(const utils::Vec3i &id_g, bool add) {
            if (!insideLocalMap(id_g)) {
                return;
            }
            utils::Vec3i neighbor_id_g;
            for (int i = -1; i <= 1; ++i) {
