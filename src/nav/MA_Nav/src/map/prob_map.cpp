...
void ProbMap::initProbMap() {
    ...
    if (cfg_.frontier_extraction_en) {
        fcnt_map_ = std::make_shared<FreeCntMap>(
            cfg_.half_map_size_i + utils::Vec3i::Constant(2),
            cfg_.resolution,
            cfg_.map_sliding_en,
            cfg_.map_sliding_thresh,
            cfg_.fix_map_origin
    ...
    logger::ProbMap->info("virtual_ceil_height: {}", cfg_.virtual_ceil_height);
    logger::ProbMap->info("virtual_ground_height: {}", cfg_.virtual_ground_height);

    if (!cfg_.map_sliding_en) {
        logger::ProbMap->warn("Map sliding disabled, set origin to [{}]", cfg_.fix_map_origin.transpose());
    ...
    resetLocalMap();
    // 遗忘衰减：时间戳初始化为当前时间，避免首帧误清全图
    occ_timestamp_buffer_.resize(map_size, utils::get_current_dir_name());
    //forget_threshold_ = (cfg_.forget_threshold > 0) ? cfg_.forget_threshold : 10.0;
    forget_threshold_ = cfg_.forget_threshold;
    logger::ProbMap->info("Init successfully");
    printMapInformation();
}

utils::Vec3f ProbMap::getLocalMapOrigin() const {
    return local_map_origin_d_;
}

utils::Vec3f ProbMap::getLocalMapSize() const {
    return cfg_.map_size_d;
}

// Query====================================================
bool ProbMap::isOccupied(const utils::Vec3f& pos) const {
...
bool ProbMap::isUnknown(const utils::Vec3f& pos) const {
    if (!insideLocalMap(pos)) {
        return true;
    }
    if (pos.z() > cfg_.virtual_ceil_height || pos.z() < cfg_.virtual_ground_height) {
...
bool ProbMap::isKnownFree(const utils::Vec3f& pos) const {
    if (!insideLocalMap(pos)) {
        return false;
    }
    if (pos.z() > cfg_.virtual_ceil_height || pos.z() < cfg_.virtual_ground_height) {
...
bool ProbMap::isFrontier(const utils::Vec3f& pos) const {
    // 1) Check local map
    if (!insideLocalMap(pos)) {
        return false;
    }
...
// ======================================================
bool ProbMap::isOccupied(const utils::Vec3i& id_g) const {
...
bool ProbMap::isUnknown(const utils::Vec3i& id_g) const {
    if (!insideLocalMap(id_g)) {
        return true;
    }
    if (id_g.z() > sc_.virtual_ceil_height_id_g || id_g.z() < sc_.virtual_ground_height_id_g + sc_.safe_margin_i) {
...
bool ProbMap::isKnownFree(const utils::Vec3i& id_g) const {
    if (!insideLocalMap(id_g)) {
        return false;
    }
    if (id_g.z() > sc_.virtual_ceil_height_id_g || id_g.z() < sc_.virtual_ground_height_id_g + sc_.safe_margin_i) {
...
bool ProbMap::isFrontier(const utils::Vec3i& id_g) const {
    // 1) Check local map
    if (!insideLocalMap(id_g)) {
        return false;
    }
...
bool ProbMap::isKnownFreeInflate(const utils::Vec3f& pos) const {
    return inf_map_->isKnownFreeInflate(pos);
}

bool ProbMap::isOccupiedInflate(const utils::Vec3f& pos) const {
    return inf_map_->isOccupiedInflate(pos);
}

bool ProbMap::isUnknownInflate(const utils::Vec3f& pos) const {
...
void ProbMap::updateOccPointCloud(const PointCloud& input_cloud) {
    ...
    utils::Vec3f localmap_min = local_map_bound_min_d_;
    utils::Vec3f localmap_max = local_map_bound_max_d_;
    for (int i = 0; i < cloud_in_size; i++) {
        static utils::Vec3f p, ray_pt;
        static utils::Vec3i pt_id_g, pt_id_l;
...
void ProbMap::slideAllMap(const utils::Vec3f& pos) {
    mapSliding(pos);
...
void ProbMap::updateProbMap(const PointCloud& cloud, const Pose& pose) {
    utils::TimeConsuming tc("updateMap", false);
    const utils::Vec3f& pos = pose.first;
    time_consuming_[4] = cloud.size();
    if (cfg_.map_sliding_en && !insideLocalMap(pos) && raycast_data_.batch_update_counter == 0) {
        logger::ProbMap->warn("cur_pose out of map range, reset the map.");
        logger::ProbMap->warn("Sliding to map center at: {}", pos.transpose());
        slideAllMap(pos);
        return;
    }

    if (pos.z() > cfg_.virtual_ceil_height) {
        logger::ProbMap->warn("Odom above virtual ceil, please check map parameter.");
        return;
    } else if (pos.z() < cfg_.virtual_ground_height) {
        logger::ProbMap->warn("Odom below virtual ground, please check map parameter.");
        return;
    }
    ...
    updateLocalBox(pos);
    utils::TimeConsuming t_raycast("raycast", false);
    ...
    if (raycast_data_.batch_update_counter >= cfg_.batch_update_size) {
        ...
        utils::TimeConsuming t_update("update", false);
        probabilisticMapFromCache();
    ...
    // 遗忘衰减：每 5 个 batch 执行一次（降频，避免每帧扫 36M 格）
    static int forget_skip_counter = 0;
    if (++forget_skip_counter >= 5) {
        forget_skip_counter = 0;
        applyForgetting();
    }
        ...
        for (double dx = -cfg_.raycast_range_min; dx <= cfg_.raycast_range_min; dx += cfg_.resolution) {
            for (double dy = -cfg_.raycast_range_min; dy <= cfg_.raycast_range_min; dy += cfg_.resolution) {
                for (double dz = -cfg_.raycast_range_min; dz <= cfg_.raycast_range_min; dz += cfg_.resolution) {
                    utils::Vec3f p(dx, dy, dz);
                    if (p.norm() <= cfg_.raycast_range_min) {
                        utils::Vec3f pp = pos + p;
...
GridType ProbMap::getGridType(const utils::Vec3f& pos) const {
...
GridType ProbMap::getInfGridType(const utils::Vec3f& pos) const {
...
double ProbMap::getMapValue(const utils::Vec3f& pos) const {
    if (!insideLocalMap(pos)) {
        return 0;
    }
...
void ProbMap::boxSearch(
    ...
    out_points.clear();
    if (map_empty_) {
        logger::ProbMap->warn("Map is empty, cannot perform box search.");
        return;
    }
    if ((_box_max - _box_min).minCoeff() <= 0) {
        logger::ProbMap->warn("Box search failed, box size is zero.");
        return;
    }
    utils::Vec3f box_min_d = _box_min, box_max_d = _box_max;
    ...
    utils::Vec3i box_min_id_g, box_max_id_g;
    posToGlobalIndex(box_min_d, box_min_id_g);
    posToGlobalIndex(box_max_d, box_max_id_g);
    utils::Vec3i box_size = box_max_id_g - box_min_id_g;
    if (gt == utils::UNKNOWN) {
...
void ProbMap::boxSearchInflate(
    ...
    const utils::GridType& gt,
    utils::vec_E<utils::Vec3f>& out_points
) const {
    inf_map_->boxSearch(box_min, box_max, gt, out_points);
}

void ProbMap::boundBoxByLocalMap(utils::Vec3f& box_min, utils::Vec3f& box_max) const {
    if ((box_max - box_min).minCoeff() <= 0) {
        box_min = box_max;
        logger::ProbMap->warn("Bound box is invalid.");
        return;
    }
...
void ProbMap::resetCell(const int& hash_id) {
    ...
    if (isOccupied(ret)) {
        ...
        inf_map_->updateGridCounter(pos, utils::OCCUPIED, utils::UNKNOWN);
        if (cfg_.esdf_en) {
            esdf_map_->updateGridCounter(pos, utils::OCCUPIED, utils::UNKNOWN);
        }
    } else if (isKnownFree(ret)) {
        /// if current state is free
        utils::Vec3f pos;
        hashIdToPos(hash_id, pos);
        inf_map_->updateGridCounter(pos, utils::KNOWN_FREE, utils::UNKNOWN);
        if (cfg_.esdf_en) {
            esdf_map_->updateGridCounter(pos, utils::KNOWN_FREE, utils::UNKNOWN);
        }
        if (cfg_.frontier_extraction_en) {
            utils::Vec3i id_g;
            posToGlobalIndex(pos, id_g);
...
void ProbMap::probabilisticMapFromCache() {
    //    int addr = getHashIndexFromGlobalIndex(utils::Vec3i(41,
    //                                                 -216,
    //                                                 -6));
    ...
    while (!raycast_data_.update_cache_id_g.empty()) {
        utils::Vec3f pos;
        utils::Vec3i id_g = raycast_data_.update_cache_id_g.front();
...
void ProbMap::hitPointUpdate(const utils::Vec3f& pos, const int& hash_id, const int& hit_num) {
    float& ret = occupancy_buffer_[hash_id];
    utils::GridType from_type = utils::UNDEFINED;

    if (isOccupied(ret)) {
        from_type = utils::GridType::OCCUPIED;
    } else if (isKnownFree(ret)) {
        from_type = utils::GridType::KNOWN_FREE;
    } else {
        from_type = utils::GridType::UNKNOWN;
    }
    ...
    utils::GridType to_type;
    if (isOccupied(ret)) {
        to_type = utils::GridType::OCCUPIED;
        occ_timestamp_buffer_[hash_id] = utils::get_current_dir_name();
    } else if (isKnownFree(ret)) {
        to_type = utils::GridType::KNOWN_FREE;
    } else {
        to_type = utils::GridType::UNKNOWN;
    }

    if (from_type != to_type) {
        utils::Vec3f center_pos;
        utils::Vec3i id_g;
        posToGlobalIndex(pos, id_g);
        ...
        if (cfg_.frontier_extraction_en && from_type == utils::KNOWN_FREE) {
            utils::Vec3i id_g;
            posToGlobalIndex(pos, id_g);
            fcnt_map_->updateFrontierCounter(id_g, false);
        }
...
void ProbMap::missPointUpdate(const utils::Vec3f& pos, const int& hash_id, const int& hit_num) {
    float& ret = occupancy_buffer_[hash_id];
    utils::GridType from_type;
    if (isOccupied(ret)) {
        from_type = utils::GridType::OCCUPIED;
    } else if (isKnownFree(ret)) {
        from_type = utils::GridType::KNOWN_FREE;
    ...
    if (from_type != to_type) {
        ...
        if (cfg_.frontier_extraction_en && to_type == utils::KNOWN_FREE) {
            utils::Vec3i id_g;
            posToGlobalIndex(pos, id_g);
...
void ProbMap::raycastProcess(const PointCloud& input_cloud, const utils::Vec3f& cur_odom) {
    // bounding box of updated region
    raycast_data_.cache_box_min = cur_odom;
    raycast_data_.cache_box_max = cur_odom;
    utils::Vec3f raycast_box_min, raycast_box_max;
    ...
    auto raycasting_cloud = utils::vec_Vec3f {};
    ...
    for (const auto& pcl_p: input_cloud) {
        ...
        utils::Vec3f p(pcl_p.x, pcl_p.y, pcl_p.z);
        utils::Vec3i pt_id_g;
    ...
    if (cfg_.raycasting_en) {
        ...
        for (const auto& p: raycasting_cloud) {
            utils::Vec3f raycast_start = (p - cur_odom).normalized() * cfg_.raycast_range_min + cur_odom;
            raycast_data_.raycaster.setInput(raycast_start, p);
            utils::Vec3f ray_pt;
            while (raycast_data_.raycaster.step(ray_pt)) {
                utils::Vec3i cur_ray_id_g;
...
void ProbMap::insertUpdateCandidate(const utils::Vec3i& id_g, bool is_hit) {
...
void ProbMap::updateLocalBox(const utils::Vec3f& cur_odom) {
    /* This function is only used for decide the local update range
     * and do not related to the map origin and map bound
     * the map origin and map bound is only update in [SlidingMap::mapSliding(const utils::Vec3f &odom)]
     * */
    ...
    utils::Vec3i cur_odom_i;
    posToGlobalIndex(cur_odom, cur_odom_i);
    utils::Vec3i local_updatebox_min_i, local_updatebox_max_i;
...
void ProbMap::resetLocalMap() {
    logger::ProbMap->warn("Clear all local map.");
...
void ProbMap::applyForgetting() {
    // forget_threshold_ <= 0 表示禁用衰减
    if (forget_threshold_ <= 0) {
        return;
    }
    double now = utils::get_current_dir_name();
    size_t cleared = 0;
    for (size_t i = 0; i < occupancy_buffer_.size(); ++i) {
        if (!isOccupied(occupancy_buffer_[i])) {
            continue;
        }
        // 时间戳为 0 说明从未被占据过（旧数据），跳过
        if (occ_timestamp_buffer_[i] < 1e-6) {
            continue;
        }
        if (now - occ_timestamp_buffer_[i] > forget_threshold_) {
            resetCell(static_cast<int>(i));
            occ_timestamp_buffer_[i] = 0;  // 清除时间戳，防止重复检查
            ++cleared;
        }
    }
    if (cleared > 0) {
        logger::ProbMap->debug("[Forget] cleared {} cells older than {:.1f}s", cleared, forget_threshold_);
    }
}