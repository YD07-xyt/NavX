...
#include "map/3d_occ_map/esdf_map.h"
#include "utils/logger.hpp"
#include <mutex>

namespace rog_map {

    void ESDFMap::initESDFMap(const utils::Vec3i &half_prob_map_size_i, const double &prob_map_resolution,
                              const double &temp_counter_map_resolution, const utils::Vec3f &local_update_box,
                              const bool &map_sliding_en, const double &sliding_thresh,
                              const utils::Vec3f &fix_map_origin, const double &unk_thresh) {
        ...
        resetLocalMap();
        logger::ESDFMap->info("Init successfully, map size: {}, sub_grid_num: {}, unk_thresh: {}.", sc_.map_vox_num, md_.sub_grid_num, md_.unk_thresh);
        printMapInformation();
    }

    void ESDFMap::getUpdatedBbox(utils::Vec3f &box_min, utils::Vec3f &box_max) const {
    ...
    void ESDFMap::resetLocalMap() {
        logger::ESDFMap->warn("Reset local map.");
    ...
    double ESDFMap::getDistance(const utils::Vec3f &pos) const {
        return distance_buffer[getHashIndexFromPos(pos)];
    }

    double ESDFMap::getDistance(const utils::Vec3i &id_g) const {
        return distance_buffer[getHashIndexFromGlobalIndex(id_g)];
    }

    void ESDFMap::updateESDF3D(const utils::Vec3f &cur_odom) {
        std::lock_guard<std::mutex> lck(update_esdf_mtx);
        using namespace std;
        utils::TimeConsuming up_t("updateESDF3D", false);
        utils::Vec3i id_l, cur_odom_i;
        posToGlobalIndex(cur_odom, cur_odom_i);
        globalIndexToLocalIndex(local_map_bound_min_i_, id_l);
        id_l = id_l + sc_.half_map_size_i;

        utils::Vec3i mem_end = sc_.map_size_i - utils::Vec3i::Ones() - id_l;
        ...
        utils::Vec3i min_esdf = cur_odom_i - half_local_update_box_i_;
        utils::Vec3i max_esdf = cur_odom_i + half_local_update_box_i_;
        ...
        double dt = up_t.stop();
        if(dt > 0.1) {
            logger::ESDFMap->info("updateESDF3D time: {}", dt);
        }
        ...
        t += up_t.stop();
        cnt++;
        logger::ESDFMap->info("Average time per point: {}us", t / cnt / sc_.map_vox_num * 1e6);
#endif
    }
    ...
    void ESDFMap::getPositiveESDFPointCloud(const utils::Vec3f &box_min_d, const utils::Vec3f &box_max_d,
                                     const double &visualize_z, pcl::PointCloud<pcl::PointXYZI> & pcl_pc) {
        std::lock_guard<std::mutex> lck(update_esdf_mtx);
        pcl_pc.clear();
        utils::Vec3i box_min_i, box_max_i;
    ...
    void ESDFMap::getNegativeESDFPointCloud(const utils::Vec3f &box_min_d, const utils::Vec3f &box_max_d,
                                     const double &visualize_z, pcl::PointCloud<pcl::PointXYZI> & pcl_pc) {
        std::lock_guard<std::mutex> lck(update_esdf_mtx);
        pcl_pc.clear();
        utils::Vec3i box_min_i, box_max_i;
    ...
    // EDT Environment
    void ESDFMap::getSurroundPts(const utils::Vec3f& pos, utils::Vec3f pts[2][2][2], utils::Vec3f & diff){
        /* interpolation position */
        utils::Vec3f pos_m = pos - 0.5 * sc_.resolution * utils::Vec3f::Ones();
