...
using Map = rog_map::vec_E<rog_map::Vec3f>;
...
public:
    ...
    auto analyze(const super_utils::RobotState& robot_pose,
...
private:
    ...
    struct GridInfo {
        int width, height;
        rog_map::Vec3f origin;   // 窗口左下角世界坐标
    };
    auto setupGrid(const rog_map::Vec3f& robot_pos) const -> GridInfo;
    auto worldToGrid(const GridInfo& grid, const rog_map::Vec3f& world) const
        -> std::optional<std::pair<int, int>>;
    auto gridToWorld(const GridInfo& grid, int gx, int gy) const -> rog_map::Vec3f;
};
