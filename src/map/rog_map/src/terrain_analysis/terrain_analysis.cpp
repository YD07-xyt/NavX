...
// auto TerrainAnalyzer::analyze(const super_utils::RobotState &robot_pose,
...
auto TerrainAnalyzer::analyze(const super_utils::RobotState &robot_pose,
  ...
  // 1. 存储每个网格内的完整点（Vec3f）
  std::vector<std::vector<rog_map::Vec3f>> points_in_grid(grid.width *
                                                          grid.height);
  for (const auto &pt : occ_map) {
    auto idx_opt = worldToGrid(grid, pt);
    if (!idx_opt)
...
auto TerrainAnalyzer::setupGrid(const rog_map::Vec3f &robot_pos) const
    -> GridInfo {
  GridInfo grid;
...
auto TerrainAnalyzer::worldToGrid(const GridInfo &grid,
                                  const rog_map::Vec3f &world) const
...
auto TerrainAnalyzer::gridToWorld(const GridInfo &grid, int gx, int gy) const
  ...
  return rog_map::Vec3f(wx, wy, 0.0f);
}
