...
namespace planner {
    ...
    private:
        // 降采样路径点
        std::vector<Point> downsamplePath(const std::vector<Point>& path, int step = 5) {
            if (path.size() <= step) return path;
            ...
            std::cout << "降采样: " << path.size() << " -> " << downsampled.size() << " 点" << std::endl;
            return downsampled;
        }
        ...
        std::vector<Point> removeDuplicates(const std::vector<Point>& path) {
            ...
            if (cleaned.size() < path.size()) {
                std::cout << "去重: " << path.size() << " -> " << cleaned.size() << " 点" << std::endl;
            }
            return cleaned;
        }
        
        // 预处理路径：去重 + 降采样
        std::vector<Point> preprocessPath(const std::vector<Point>& path, int downsample_step = 5) {
            // 1. 先去重
    ...
    public:
        ...
        bool optimization(int downsample_step = 5) {
            ...
            std::cout << "起点: (" << processed_path.front().x << ", " << processed_path.front().y << ")" << std::endl;
            std::cout << "终点: (" << processed_path.back().x << ", " << processed_path.back().y << ")" << std::endl;
            ...
            std::cout << "优化成功！轨迹段数: " << opt_trajectory.getPieceNum() << std::endl;
            std::cout << "总时长: " << opt_trajectory.getTotalDuration() << " 秒" << std::endl;
