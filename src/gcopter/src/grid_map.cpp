...
GridMap2D::GridMap2D(double resolution, const Eigen::Vector2i &world_size)
    : resolution_(resolution), inflate_radius_(0.5),
      world_size_(world_size) { // 默认膨胀半径0.5米
}

void GridMap2D::setCurPose(double x, double y) {
  // 1. 校验输入合法性
  if (resolution_ <= 0) {
    throw std::invalid_argument("地图分辨率必须为正数（当前：" +
                                std::to_string(resolution_) + "）");
  }
  if (world_size_.x() <= 0 || world_size_.y() <= 0) {
    throw std::invalid_argument("世界尺寸(x,y)必须为正数(当前：(" +
                                std::to_string(world_size_.x()) + "," +
                                std::to_string(world_size_.y()) + "))");
  }

  // 2. 计算栅格数量（世界尺寸 / 分辨率，向上取整确保覆盖完整世界范围）
  int grid_cols = static_cast<int>(std::ceil(
      static_cast<double>(world_size_.x()) / resolution_)); // x方向→列数
  int grid_rows = static_cast<int>(std::ceil(
      static_cast<double>(world_size_.y()) / resolution_)); // y方向→行数
  ...
  origin_.x() =
      x - static_cast<double>(world_size_.x()) /
              2.0; // 世界x范围：[origin.x(), origin.x()+world_size.x()]
  origin_.y() =
      y - static_cast<double>(world_size_.y()) /
              2.0; // 世界y范围：[origin.y(), origin.y()+world_size.y()]

  // 4. 初始化网格（原始障碍物网格 + 膨胀后网格）：默认全为自由空间（false）
  original_grid_.resize(grid_rows,
                        std::vector<bool>(grid_cols, false)); // 存储原始障碍物
  grid_.resize(grid_rows,
               std::vector<bool>(grid_cols, false)); // 存储原始+膨胀障碍物

  // 打印初始化信息
  // spdlog::info(" [GridMap2D] 地图初始化完成：");
  // spdlog::info("  - 世界尺寸:{} m x {}m", world_size_.x(), world_size_.y());
  // spdlog::info("  - 栅格分辨率:{}m/栅格", resolution_);
  // spdlog::info("栅格数量：{}列 x {}行 ", grid_cols, grid_rows);
  // spdlog::info("  - 世界坐标范围：x∈[{:.2f}, {:.2f}], y∈[{:.2f}, {:.2f}]",
  //              origin_.x(), origin_.x() + world_size_.x(), origin_.y(),
  //              origin_.y() + world_size_.y());
  // spdlog::info("  - 默认膨胀半径：{}m", inflate_radius_);
}
...
void GridMap2D::setObstacle(const Eigen::Vector2i &grid_index,
                            bool is_obstacle) {
  // 1. 校验栅格索引合法性
  if (!isIndexValid(grid_index)) {
    spdlog::warn("[setObstacle] 警告：栅格索引（{}, {}）超出范围（最大：({}, "
                 "{})），不执行设置",
                 grid_index.x(), grid_index.y(), map_size_.x() - 1,
                 map_size_.y() - 1);
  ...
  // spdlog::info("[setObstacle] 栅格（{}, {}）设置为{}", grid_index.x(),
  //              grid_index.y(), (is_obstacle ? "障碍物" : "自由空间"));
}
...
bool GridMap2D::isObstacle(const Eigen::Vector2d &world_pos) const {
  ...
  // 2. 超出地图范围视为障碍物（避免越界）
  if (!isIndexValid(grid_index)) {
    // std::cout << "[isObstacle] 点（" << world_pos.x() << "," << world_pos.y()
    // << "）超出地图范围，视为原始障碍物" << std::endl;
    return true;
  }
  ...
  // std::cout << "[isObstacle] 点（" << world_pos.x() << "," << world_pos.y()
  // << "）→ 栅格（"
  //           << grid_index.x() << "," << grid_index.y() <<
  //           "），原始障碍物状态：" << is_obs << std::endl;
  return is_obs;
}
// 获取障碍物点云并转化为世界坐标
std::vector<Eigen::Vector3d>
GridMap2D::getObstaclePointCloud3d(bool return_inflated_map) const {
  ...
  for (int row = 0; row < map_size_.y(); ++row) {
    for (int col = 0; col < map_size_.x(); ++col) {
      ...
      bool is_obs =
          return_inflated_map ? grid_[row][col] : original_grid_[row][col];
      ...
      if (is_obs) {
        ...
        point_cloud.emplace_back(world_x, world_y,1.0);
      }
    }
  }

  // std::cout << "[getObstaclePointCloud] 生成点云数量：" << point_cloud.size()
  //           << " (模式：" << (return_inflated_map ? "膨胀后" : "原始") << ")"
  //           << std::endl;
...
GridMap2D::getObstaclePointCloud(bool return_inflated_map) const {
...
bool GridMap2D::getInflateOccupancy(const Eigen::Vector2d &world_pos) const {
  ...
  if (!isIndexValid(grid_index)) {
    std::cout << "[getInflateOccupancy] 点（" << world_pos.x() << ","
              << world_pos.y() << "）超出地图范围，视为膨胀占用" << std::endl;
    return true;
  }
...
void GridMap2D::inflateObstacles(double radius) {
  // 1. 校验膨胀半径
  if (radius <= 0) {
    std::cerr << "[inflateObstacles] 警告：膨胀半径必须为正数（当前：" << radius
              << "），不执行膨胀" << std::endl;
    return;
  }

  // std::cout << "\n[inflateObstacles] 开始膨胀障碍物，半径：" << radius << "m"
  //           << std::endl;

  // 2. 计算膨胀半径对应的栅格数（向上取整，确保覆盖完整半径）
  int inflate_grid_num = static_cast<int>(std::ceil(radius / resolution_));
  // std::cout << "  - 膨胀半径对应栅格数：" << inflate_grid_num << "（" << radius
  //           << "m / " << resolution_ << "m/栅格）" << std::endl;
  ...
  // std::cout << "  - 原始障碍物数量：" << original_obstacle_indices.size()
  //           << "个栅格" << std::endl;
  ...
  for (const auto &obs_idx : original_obstacle_indices) {
    ...
    for (int dr = -inflate_grid_num; dr <= inflate_grid_num;
         ++dr) { // 行方向偏移
      for (int dc = -inflate_grid_num; dc <= inflate_grid_num;
           ++dc) { // 列方向偏移
        // 计算当前待检查的栅格索引
  ...
  // std::cout << "  - 膨胀后总占用栅格数："
  //           << (original_obstacle_indices.size() + inflated_count) << "个"
  //           << std::endl;
}
...
void GridMap2D::setInflateRadius(double radius) {
  if (radius <= 0) {
    std::cerr << "[setInflateRadius] 警告：膨胀半径必须为正数（当前输入："
              << radius << "），不修改默认半径" << std::endl;
    return;
  }
  inflate_radius_ = radius;
  std::cout << "[setInflateRadius] 成功更新默认膨胀半径：" << radius
            << "m（原：" << inflate_radius_ << "m）" << std::endl;
}
...
Eigen::Vector2i GridMap2D::worldToGrid(const Eigen::Vector2d &world_pos) const {
  ...
  grid_index.x() =
      static_cast<int>(std::round((world_pos.x() - origin_.x()) / resolution_));
  grid_index.y() =
      static_cast<int>(std::round((world_pos.y() - origin_.y()) / resolution_));
  // std::cout << "[worldToGrid] 世界坐标（" << world_pos.x() << "," <<
  // world_pos.y() << "）→ 栅格索引（"
...
Eigen::Vector2d
GridMap2D::gridToWorld(const Eigen::Vector2i &grid_index) const {
  // 校验索引合法性
  if (!isIndexValid(grid_index)) {
    std::cerr << "[gridToWorld] 警告：栅格索引（" << grid_index.x() << ","
              << grid_index.y() << "）超出范围，返回原点坐标" << std::endl;
    return origin_;
  }
  ...
  // std::cout << "[gridToWorld] 栅格索引（" << grid_index.x() << "," <<
  // grid_index.y() << "）→ 世界坐标（"
...
bool GridMap2D::isIndexValid(const Eigen::Vector2i &grid_index) const {
  ...
  if (!valid) {
    std::cerr << "[isIndexValid] 栅格索引（" << grid_index.x() << ","
              << grid_index.y() << "）无效！地图范围：列[0,"
              << map_size_.x() - 1 << "], 行[0," << map_size_.y() - 1 << "]"
              << std::endl;
  }
  return valid;
}
...
//     std::cout << "\n======================= 地图可视化
//     =======================" << std::endl; std::cout << "  地图类型：" <<
//     (print_inflated ? "膨胀后障碍物（#=原始/膨胀，.=自由）" :
//     "原始障碍物（#=原始，.=自由/膨胀）") << std::endl; std::cout << "
//     栅格尺寸：" << map_size_.x() << "列 × " << map_size_.y() << "行" <<
//     std::endl; std::cout <<
//     "==========================================================" <<
//     std::endl;
...
//         for (int col = 0; col < map_size_.x(); ++col) {
//             // 选择要打印的网格（原始/膨胀后）
//             bool is_occupied = print_inflated ? grid_[row][col] :
//             original_grid_[row][col]; std::cout << (is_occupied ? "# " : ".
//             ");  // 障碍物→#，自由→.
//         }
...
// Getter：获取分辨率
double GridMap2D::resolution() const { return resolution_; }

// Getter：获取栅格尺寸（列数x行数）
const Eigen::Vector2i &GridMap2D::mapSize() const { return map_size_; }

// Getter：获取世界坐标原点
const Eigen::Vector2d &GridMap2D::origin() const { return origin_; }

double GridMap2D::getResolution() { return resolution_; }
