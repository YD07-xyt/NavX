...
public:
    ...
    inline std::vector<std::vector<int>> get_grid(){
        std::vector<std::vector<int>> grid_int(grid_rows_, std::vector<int>(grid_cols_, 0));
        for(size_t i=0;i<grid_.size();i++){
            for(size_t j=0;j<grid_.size();j++){
                if(grid_[i][j]==false){
                    grid_int[i][j]=0;
                }else if (grid_[i][j]==true){
                     grid_int[i][j]=1;
                }
            }
        }
        spdlog::info("get_grid success");
        return grid_int;
    }
    ...
    void resetMap()
    {
        ...
        if (grid_rows_ > 0 && grid_cols_ > 0) {
            ...
            spdlog::info("[resetMap] 所有栅格已重置为自由空间。");
            
        } else {
            spdlog::info("[resetMap] 警告：地图尚未初始化，无法重置。");
        }
    }
    ...
    inline Eigen::Vector2d getCorner(void) const {
        return origin_ + map_size_.cast<double>() * resolution_;
    }
    ...
    std::vector<Eigen::Vector3d> getObstaclePointCloud3d(bool return_inflated_map) const ;
    // Getter
